...
export function berechnePrivatVergleichErgebnis(state: BetriebState): PrivatVergleichErgebnis {
  ...
  for (let jahr = 1; jahr <= state.laufzeitJahre; jahr++) {
    ...
    const simulierterGewinnPrivat = Math.max(0, state.simulierterGewinn ?? 0);
    ...
    const sparplanNetto = jaehrlicherCashZuschuss + simulierterGewinnPrivat + darlehensZuschussJaehrlich - konsumNutzenwert;
